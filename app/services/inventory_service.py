...
class InventoryService(BaseService[Inventory]):
    ...
    async def get_available_inventory(self, film_id: int, store_id: int | None = None, page: int = 1, size: int = 20):
        ...
        count_q = select(func.count()).select_from(Inventory).where(
            Inventory.film_id == film_id,
            Inventory.inventory_id.not_in(rented_subq),
        )
        if store_id:
    ...
    async def get_store_inventory(self, store_id: int, page: int = 1, size: int = 20, sort_by: str | None = None, order: str = "asc"):
